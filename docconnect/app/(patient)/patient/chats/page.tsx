...
export default function PatientChatsPage() {
  ...
  async function loadChats() {
    ...
    const { data: sessions, error } = await supabase
      .from('chat_sessions')
      .select('id, status, session_type, created_at, updated_at, doctor_id')
    ...
    const doctorIds = [...new Set(sessions.map((s: { doctor_id: string }) => s.doctor_id))]
    const { data: doctorProfiles } = await supabase
      .from('doctor_profiles')
      .select('id, user_id')
      .in('id', doctorIds)

    const userIds = (doctorProfiles ?? []).map((dp: { user_id: string }) => dp.user_id)
    ...
    // Build lookup maps
    const doctorMap = new Map((doctorProfiles ?? []).map((dp: { id: string; user_id: string }) => [dp.id, dp]))
    ...
    for (const msg of (latestMsgs ?? [])) {
      if (!lastMsgMap.has(msg.session_id)) {
        lastMsgMap.set(msg.session_id, msg.content)
      }
    }

    const items: ChatItem[] = sessions.map((s: { id: string; status: ChatItem['status']; session_type: ChatItem['session_type']; created_at: string; updated_at: string; doctor_id: string }) => {
      const dp = doctorMap.get(s.doctor_id) as { user_id: string } | undefined
      const profile = dp ? profileMap.get(dp.user_id) as { full_name: string; avatar_url: string | null } | undefined : undefined
      return {
        id: s.id,
        status: s.status,
        ...
        doctorAvatarUrl: profile?.avatar_url ?? null,
    ...
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-6 max-w-3xl mx-auto w-full">
      <div className="mb-6">
        ...
        <p className="text-sm mt-1" style={{ color: '#9CA3AF' }}>
          All your chat sessions with doctors
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl bg-white p-10 text-center" style={{ border: '1.5px solid #E5E7EB' }}>
          ...
          <p className="font-semibold" style={{ color: '#1C1917' }}>No consultations yet</p>
          <p className="text-sm mt-1" style={{ color: '#9CA3AF' }}>
            Find a doctor and start your first consultation.
          </p>
          <Link
            href="/patient/dashboard?tab=find-doctor"
            className="inline-block mt-4 px-4 py-2 rounded-lg text-sm font-semibold text-white hover:opacity-90 transition-opacity"
        ...
        <div className="space-y-2">
          {chats.map((chat) => {
            const initials = chat.doctorName.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase()
            return (
              <div
                key={chat.id}
              ...
              >
                ...
                <div className="shrink-0">
                  {chat.doctorAvatarUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                      src={chat.doctorAvatarUrl}
                      alt={chat.doctorName}
                ...
                {/* Info */}
                <Link href={`/patient/chat/${chat.id}`} className="flex-1 min-w-0">
                  ...
                  <p className="text-xs truncate mt-0.5" style={{ color: '#9CA3AF' }}>
                    {chat.lastMessage ?? (chat.status === 'pending' ? 'Waiting for doctor response…' : 'No messages yet')}
                  </p>
